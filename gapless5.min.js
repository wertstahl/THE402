...
function Gapless5Source(parentPlayer, parentLog, inAudioPath) {
  ...
  const setEndedCallbackTime = (restSecNormalized) => {
    ...
    endedCallback = window.setTimeout(onEnded, (restSec * 1000) - player.callbackCheat); // THE402 EDIT
  };
...
function Gapless5FileList(parentPlayer, parentLog, inShuffle, inLoadLimit = -1, inTracks = [], inStartingTrack = 0) {
  ...
  this.gotoTrack = (pointOrPath, forcePlay, allowOverride, keepPrevAlive) => { // THE402 EDIT
    ...
    window.setTimeout(killPrev, keepPrevAlive ? player.callbackCheat : 0);
    // THE402 END
...
function Gapless5(options = {}, deprecated = {}) { // eslint-disable-line no-unused-vars
  ...
  this.callbackCheat = options.callbackCheat || 0; // THE402 EDIT
    ...
    if (source) {
      ...
      if (this.queuedTrack) {
        ...
        if (this.singleMode || this.totalTracks() === 1) {
          ...
          source.stop();
          this.next(null, true, true); // THE402 EDIT
        }
      } else {
        source.stop();
        this.scrub(0, true);
  ...
  this.gotoTrack = (pointOrPath, forcePlay, allowOverride = false, keepPrevAlive = false) => { // THE402 EDIT
    if (!isValidIndex(this.playlist.indexFromTrack(pointOrPath))) {
      log.error(`Cannot go to missing track: ${pointOrPath}`);
    } else {
      const newIndex = this.playlist.gotoTrack(pointOrPath, forcePlay, allowOverride, keepPrevAlive); // THE402 EDIT
  ...
  this.next = (_uiEvent, forcePlay, keepPrevAlive) => { // THE402 EDIT
    const currentSource = this.currentSource();
    if (!currentSource) {
      return;
    }
    ...
    this.gotoTrack(track, forcePlay || this.isPlaying(), true, keepPrevAlive); // THE402 EDIT
    const newSource = this.currentSource();
